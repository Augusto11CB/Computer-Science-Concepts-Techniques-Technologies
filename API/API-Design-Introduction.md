...
## Consideration
The content written in this material is the main points explained in the article An Introduction to APIs by Brian Cooksey.
...
Real-time communication will be split into two broad categories 1.  The ﬁrst we call "client-driven," is where a person interacts with the client and wants the server's data to update. The other we call "server-driven", where a person does something on the server and needs the client to be aware of the change.

### Client-driven integration
When a user interacts with the client, the client knows exactly when data changes, so it can call the API  immediately to let the server know. There's no delay (hence it's real-time) and the process is eﬃcient because only one request is made for each action taken by a person.

### Server Driven integration
The data is changing on the server and the client needs to know about it. Yet, if the server can't  make requests, since it
...
Is a process dynamic that does not require a person to manually enter a Callback URL on the server. Examples: HTTP Subscriptions Speciﬁcation, Restful Webhooks,  REST Hooks, and PubSubHubbub.
...
* Identificação dos recursos (http://site.com/cliente/57). **Cuidado** com IDs sequenciais, pois é possível ler toda a base.
...
- API HATEOS provê informações que permite navegar entre seus endpoints de forma dinâmica visto que inclui links junto às respostas
...
As APIs são a forma primária como um produto expõe suas funcionalidades e por isso é tão importante que elas sejam desenhadas de forma inteligente.
...
- Reduz a probabilidade de erros na integração por trocas de payload. APIs desenvolvidas antes do código tendem a serem melhor desenhadas, confiáveis, consistentes e fáceis de utilizar.

### How to build my API?
1 - Brainstorm para entender o problema que precisa ser resolvido, as características do sistema, as regras de negócio que serão tratadas, os casos de uso, o domínio a ser trabalhado e sua interface com outros domínios

2 - Utilizar o conceito de API First e documentação. Assim as outras pessoas já vão saber como se integrar

3 - Criar testes de contratos com endpoints, versionamento, Stubs dos requisitos e responsáveis e códigos de erros. Não se esqueça de seguir o style guide de governança
